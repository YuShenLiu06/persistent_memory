...
Hooks 是用户定义的 shell 命令、HTTP 端点或 LLM 提示，在 Claude Code 生命周期中的特定点自动执行。使用此参考来查找事件架构、配置选项、JSON 输入/输出格式以及异步 hooks、HTTP hooks 和 MCP 工具 hooks 等高级功能。如果您是第一次设置 hooks，请改为从 [指南](/zh-CN/hooks-guide) 开始。
...
<Steps>
  ...
  <Step title="匹配器检查">
    匹配器 `"Bash"` 与工具名称匹配，因此 `block-rm.sh` 运行。如果您省略匹配器或使用 `"*"`，hook 在事件的每次出现时运行。只有当定义了匹配器且不匹配时，hooks 才会跳过。
  </Step>

  <Step title="Hook 处理程序运行">
    该脚本从输入中提取 `"rm -rf /tmp/build"` 并找到 `rm -rf`，因此它将决定打印到 stdout：
...
错误处理与命令 hooks 不同：非 2xx 响应、连接失败和超时都会产生非阻止错误，允许执行继续。要阻止工具调用或拒绝权限，返回 2xx 响应，其 JSON 体包含 `decision: "block"` 或包含 `permissionDecision: "deny"` 的 `hookSpecificOutput`。
...
| 字段       | 必需 | 描述                                               |
| :------- | :- | :----------------------------------------------- |
| `prompt` | 是  | 要发送到模型的提示文本。使用 `$ARGUMENTS` 作为 hook 输入 JSON 的占位符 |
...
在 Claude Code 中键入 `/hooks` 以打开交互式 hooks 管理器，您可以在其中查看、添加和删除 hooks，而无需直接编辑设置文件。有关分步演练，请参阅指南中的 [设置您的第一个 hook](/zh-CN/hooks-guide#set-up-your-first-hook)。
...
要移除 hook，请从设置 JSON 文件中删除其条目，或使用 `/hooks` 菜单并选择 hook 以删除它。

要临时禁用所有 hooks 而不移除它们，请在设置文件中设置 `"disableAllHooks": true` 或使用 `/hooks` 菜单中的切换。没有办法在保持 hook 在配置中的同时禁用单个 hook。

`disableAllHooks` 设置遵守托管设置层次结构。如果管理员已通过托管策略设置配置了 hooks，则在用户、项目或本地设置中设置的 `disableAllHooks` 无法禁用这些托管 hooks。只有在托管设置级别设置的 `disableAllHooks` 才能禁用托管 hooks。

对设置文件中 hooks 的直接编辑不会立即生效。Claude Code 在启动时捕获 hooks 的快照，并在整个会话中使用它。这可以防止恶意或意外的 hook 修改在会话中途生效而不经过您的审查。如果 hooks 在外部被修改，Claude Code 会警告您，并要求在 `/hooks` 菜单中进行审查，然后更改才能应用。
...
`tool_name` 和 `tool_input` 字段是事件特定的。每个 [hook 事件](#hook-events) 部分记录了该事件的其他字段。

### 退出代码输出

来自您的 hook 命令的退出代码告诉 Claude Code 该操作是否应该继续、被阻止或被忽略。

**退出 0** 表示成功。Claude Code 解析 stdout 以查找 [JSON 输出字段](#json-output)。JSON 输出仅在退出 0 时处理。对于大多数事件，stdout 仅在详细模式（`Ctrl+O`）中显示。例外是 `UserPromptSubmit` 和 `SessionStart`，其中 stdout 被添加为 Claude 可以看到和作用的上下文。
...
退出代码让您允许或阻止，但 JSON 输出给您更细粒度的控制。与其以代码 2 退出来阻止，不如以 0 退出并将 JSON 对象打印到 stdout。Claude Code 从该 JSON 读取特定字段以控制行为，包括 [决定控制](#decision-control) 来阻止、允许或升级给用户。

<Note>
  您必须为每个 hook 选择一种方法，而不是两种：要么单独使用退出代码进行信号传递，要么以 0 退出并打印 JSON 以进行结构化控制。Claude Code 仅在退出 0 时处理 JSON。如果您以 2 退出，任何 JSON 都会被忽略。
</Note>
...
| `stopReason`     | 无       | hook 运行后向用户显示的消息。不向 Claude 显示                      |
...
| TeammateIdle、TaskCompleted                                                     | 仅退出代码                | 退出代码 2 阻止操作，stderr 作为反馈反馈                                       |
| PreToolUse                                                                     | `hookSpecificOutput` | `permissionDecision`（allow/deny/ask）、`permissionDecisionReason` |
| PermissionRequest                                                              | `hookSpecificOutput` | `decision.behavior`（allow/deny）                                 |
| WorktreeCreate                                                                 | stdout 路径            | Hook 打印创建的 worktree 的绝对路径。非零退出失败创建                              |
| WorktreeRemove、Notification、SessionEnd、PreCompact                              | 无                    | 无决定控制。用于日志记录或清理等副作用                                             |

以下是每种模式的实际示例：

<Tabs>
  <Tab title="顶级决定">
    由 `UserPromptSubmit`、`PostToolUse`、`PostToolUseFailure`、`Stop`、`SubagentStop` 和 `ConfigChange` 使用。唯一的值是 `"block"`。要允许操作继续，从您的 JSON 中省略 `decision`，或以 0 退出而不带任何 JSON：
  ...
  <Tab title="PreToolUse">
    使用 `hookSpecificOutput` 以获得更丰富的控制：允许、拒绝或升级给用户。您还可以在运行前修改工具输入或为 Claude 注入其他上下文。有关完整的选项集，请参阅 [PreToolUse 决定控制](#pretooluse-decision-control)。
...
每个事件对应于 Claude Code 生命周期中 hooks 可以运行的一个点。下面的部分按顺序排列以匹配生命周期：从会话设置通过代理循环到会话结束。每个部分描述事件何时触发、它支持什么匹配器、它接收的 JSON 输入以及如何通过输出控制行为。

### SessionStart

在 Claude Code 启动新会话或恢复现有会话时运行。用于加载开发上下文，如现有问题或代码库的最近更改，或设置环境变量。对于不需要脚本的静态上下文，请改用 [CLAUDE.md](/zh-CN/memory)。

SessionStart 在每个会话上运行，所以保持这些 hooks 快速。
...
您的 hook 脚本打印到 stdout 的任何文本都被添加为 Claude 的上下文。除了 [JSON 输出字段](#json-output) 对所有 hooks 可用外，您还可以返回这些事件特定字段：
...
<Note>
  `CLAUDE_ENV_FILE` 对 SessionStart hooks 可用。其他 hook 类型无法访问此变量。
</Note>

### UserPromptSubmit

在用户提交提示时运行，在 Claude 处理它之前。这允许您根据提示/对话添加其他上下文、验证提示或阻止某些类型的提示。
...
* **纯文本 stdout**：写入 stdout 的任何非 JSON 文本都被添加为上下文
* **带 `additionalContext` 的 JSON**：使用下面的 JSON 格式以获得更多控制。`additionalContext` 字段被添加为上下文

纯 stdout 在成绩单中显示为 hook 输出。`additionalContext` 字段被更谨慎地添加。
...
在 Claude 创建工具参数后和处理工具调用之前运行。在工具名称上匹配：`Bash`、`Edit`、`Write`、`Read`、`Glob`、`Grep`、`Task`、`WebFetch`、`WebSearch` 和任何 [MCP 工具名称](#match-mcp-tools)。
...
| `description`       | string  | `"Run test suite"` | 命令执行内容的可选描述   |
...
| `allowed_domains` | array  | `["docs.example.com"]`         | 可选：仅包括来自这些域的结果 |
...
`PermissionRequest` hooks 可以允许或拒绝权限请求。除了 [JSON 输出字段](#json-output) 对所有 hooks 可用外，您的 hook 脚本可以返回一个 `decision` 对象，其中包含这些事件特定字段：
...
`PostToolUse` hooks 可以在工具执行后向 Claude 提供反馈。除了 [JSON 输出字段](#json-output) 对所有 hooks 可用外，您的 hook 脚本可以返回这些事件特定字段：

| 字段                     | 描述                                         |
| :--------------------- | :----------------------------------------- |
| `decision`             | `"block"` 提示 Claude 使用 `reason`。省略以允许操作继续  |
...
在工具执行失败时运行。此事件对于抛出错误或返回失败结果的工具调用触发。使用此来记录失败、发送警报或向 Claude 提供纠正反馈。
...
`PostToolUseFailure` hooks 可以在工具失败后向 Claude 提供上下文。除了 [JSON 输出字段](#json-output) 对所有 hooks 可用外，您的 hook 脚本可以返回这些事件特定字段：

| 字段                  | 描述                    |
| :------------------ | :-------------------- |
| `additionalContext` | Claude 要与错误一起考虑的其他上下文 |
...
除了 [通用输入字段](#common-input-fields) 外，Notification hooks 还接收包含通知文本的 `message`、可选的 `title` 和指示哪个类型触发的 `notification_type`。
...
Notification hooks 无法阻止或修改通知。除了 [JSON 输出字段](#json-output) 对所有 hooks 可用外，您可以返回 `additionalContext` 来向对话添加上下文：
...
在通过 Task 工具生成 Claude Code subagent 时运行。支持匹配器来按代理类型名称过滤（内置代理如 `Bash`、`Explore`、`Plan` 或来自 `.claude/agents/` 的自定义代理名称）。

#### SubagentStart 输入

除了 [通用输入字段](#common-input-fields) 外，SubagentStart hooks 还接收包含 subagent 唯一标识符的 `agent_id` 和包含代理名称的 `agent_type`（内置代理如 `"Bash"`、`"Explore"`、`"Plan"` 或自定义代理名称）。
...
SubagentStart hooks 无法阻止 subagent 创建，但它们可以向 subagent 注入上下文。除了 [JSON 输出字段](#json-output) 对所有 hooks 可用外，您可以返回：
...
`Stop` 和 `SubagentStop` hooks 可以控制 Claude 是否继续。除了 [JSON 输出字段](#json-output) 对所有 hooks 可用外，您的 hook 脚本可以返回这些事件特定字段：
...
在 [代理团队](/zh-CN/agent-teams) 队友在完成其轮次后即将空闲时运行。使用此来在队友停止工作之前强制执行质量门，例如要求通过 lint 检查或验证输出文件存在。

当 `TeammateIdle` hook 以代码 2 退出时，队友接收 stderr 消息作为反馈并继续工作而不是空闲。TeammateIdle hooks 不支持匹配器，在每次出现时触发。
...
TeammateIdle hooks 仅使用退出代码，不使用 JSON 决定控制。此示例检查构建工件是否存在，然后允许队友空闲：
...
在任务被标记为已完成时运行。这在两种情况下触发：当任何代理通过 TaskUpdate 工具显式标记任务为已完成时，或当 [代理团队](/zh-CN/agent-teams) 队友完成其轮次且有进行中的任务时。使用此来强制执行完成标准，如通过测试或 lint 检查，然后任务才能关闭。

当 `TaskCompleted` hook 以代码 2 退出时，任务不被标记为已完成，stderr 消息作为反馈反馈给模型。TaskCompleted hooks 不支持匹配器，在每次出现时触发。
...
TaskCompleted hooks 仅使用退出代码，不使用 JSON 决定控制。此示例运行测试并在失败时阻止任务完成：
...
ConfigChange hooks 可以阻止配置更改生效。使用退出代码 2 或 JSON `decision` 来防止更改。被阻止时，新设置不会应用于运行中的会话。
...
`policy_settings` 更改无法被阻止。Hooks 仍然对 `policy_settings` 源触发，因此您可以使用它们进行审计日志记录，但任何阻止决定都会被忽略。这确保企业管理的设置总是生效。
...
此示例创建 SVN 工作副本并打印路径供 Claude Code 使用。用您自己的替换存储库 URL：
...
WorktreeCreate hooks 不使用标准的允许/阻止决定模型。相反，hook 的成功或失败决定了结果。仅支持 `type: "command"` hooks。

### WorktreeRemove

[WorktreeCreate](#worktreecreate) 的清理对应物。此 hook 在 worktree 被移除时触发，要么当您退出 `--worktree` 会话并选择移除它时，要么当具有 `isolation: "worktree"` 的 subagent 完成时。对于基于 git 的 worktrees，Claude 使用 `git worktree remove` 自动处理清理。如果您为非 git 版本控制系统配置了 WorktreeCreate hook，将其与 WorktreeRemove hook 配对以处理清理。没有它，worktree 目录会留在磁盘上。
...
在 Claude Code 会话结束时运行。用于清理任务、记录会话统计或保存会话状态。支持匹配器来按退出原因过滤。
...
除了 [通用输入字段](#common-input-fields) 外，SessionEnd hooks 还接收指示会话为何结束的 `reason` 字段。有关所有值，请参阅上面的 [原因表](#sessionend)。
...
除了 Bash 命令 hooks（`type: "command"`），Claude Code 还支持基于提示的 hooks（`type: "prompt"`），使用 LLM 来评估是否允许或阻止操作，以及代理 hooks（`type: "agent"`），生成一个可以在返回决定前使用 Read、Grep 和 Glob 等工具的代理验证器。并非所有事件都支持每种 hook 类型。

支持所有三种 hook 类型（`command`、`prompt` 和 `agent`）的事件：
...
此 `Stop` hook 要求 LLM 在允许 Claude 完成之前评估是否所有任务都已完成：
...
| :-------- | :- | :------------------------------------------------------------------------------------- |
| `type`    | 是  | 必须是 `"prompt"`                                                                         |
| `prompt`  | 是  | 要发送到 LLM 的提示文本。使用 `$ARGUMENTS` 作为 hook 输入 JSON 的占位符。如果 `$ARGUMENTS` 不存在，输入 JSON 被追加到提示 |
| `model`   | 否  | 用于评估的模型。默认为快速模型                                                                        |
| `timeout` | 否  | 超时（秒）。默认：30                                                                            |
...
| `reason` | 当 `ok` 为 `false` 时需要。向 Claude 显示的解释 |

### 示例：多条件 Stop hook

此 `Stop` hook 使用详细提示来检查三个条件，然后允许 Claude 停止。如果 `"ok"` 为 `false`，Claude 继续工作，提供的原因作为其下一条指令。`SubagentStop` hooks 使用相同的格式来评估 [subagent](/zh-CN/sub-agents) 是否应该停止：
...
基于代理的 hooks（`type: "agent"`）类似于基于提示的 hooks，但具有多轮工具访问。与单个 LLM 调用不同，代理 hook 生成一个可以读取文件、搜索代码和检查代码库来验证条件的 subagent。代理 hooks 支持与基于提示的 hooks 相同的事件。

### 基于代理的 Hooks 如何工作

当代理 hook 触发时：

1. Claude Code 生成一个带有您的提示和 hook 的 JSON 输入的 subagent
...
当异步 hook 触发时，Claude Code 启动 hook 进程并立即继续而不等待它完成。hook 通过 stdin 接收与同步 hook 相同的 JSON 输入。

后台进程退出后，如果 hook 产生了带有 `systemMessage` 或 `additionalContext` 字段的 JSON 响应，该内容在下一个对话轮次作为上下文传递给 Claude。

### 示例：文件更改后运行测试

此 hook 在 Claude 写入文件时在后台启动测试套件，然后在测试完成时向 Claude 报告结果。将此脚本保存到项目中的 `.claude/hooks/run-tests-async.sh` 并使用 `chmod +x` 使其可执行：
...
* 异步 hooks 无法阻止工具调用或返回决定。当 hook 完成时，触发的操作已经进行。
...
<Warning>
  命令 hooks 以您的完整用户权限执行 shell 命令。它们可以修改、删除或访问您的用户帐户可以访问的任何文件。在将所有 hook 命令添加到您的配置之前，请审查和测试它们。
</Warning>
